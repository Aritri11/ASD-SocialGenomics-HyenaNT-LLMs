...
def run_cv_evaluation(name, pipeline, pairs, y, embed_dim,
                      n_splits=N_FOLDS):
    """
    Stratified K-fold CV with streaming mean pooling.

    Per fold:
      1. Split (path, label) index into train/test — no data loaded yet
      2. Stream-extract mean features for train + test
      3. Fit RF/SVM/XGBoost on train features
      4. Evaluate on test features
      5. Free all large arrays before next fold

    RAM at any point: O(batch_size × variants) not O(all_subjects × variants)
    """
    print(f"\n{'=' * 60}")
    print(f"Model: {name}  (streaming mean pooling)")
    print(f"{'=' * 60}")
    ...
    for fold, (tr_idx, te_idx) in enumerate(skf.split(indices, y)):
        ...
        # ── step 1: extract features (streams from disk, computes mean) ──
        # Extract features with error handling
        X_tr, tr_valid_idx, tr_skipped = extract_mean_features(tr_pairs)
        X_te, te_valid_idx, te_skipped = extract_mean_features(te_pairs)

        # Filter labels to match valid samples
        y_tr = y_tr[tr_valid_idx]
        y_te = y_te[te_valid_idx]

        print(f"  Fold {fold + 1}: Skipped {tr_skipped} train, {te_skipped} test files")

        # ── step 2: per-fold XGBoost imbalance adjustment ──
        ...
        # ── step 4: free large arrays explicitly ──
        del X_tr, X_te
...
if __name__ == "__main__":
    ...
    plot_roc_curves(
        results,
        save_path=os.path.join(OUT_DIR, "roc_curves_ml_mean.png")
    )
    plot_confusion_matrices(
        results,
        save_path=os.path.join(OUT_DIR, "confusion_matrices_ml_mean.png")
    )
    save_summary(
        results,
        save_path=os.path.join(OUT_DIR, "ml_results_summary_mean.csv")
    )
